    ...
    public class ChecksumRunnable implements Runnable
    {
        ...
        public void run()
        {
            ...
            try {
                while (true)
                {
                    FiverFile currentFile = checksumFiles.poll(1000, TimeUnit.MILLISECONDS);
                    if(currentFile==null)
                        break;
                    ...
                    while (remaining > 0)
                    {
                        ...
                        while (processedBytes < currentBlockSize)
                        {
                            while(items.size()==0)
                                Thread.sleep(10);
                            item = items.poll();
                            //item=updateQueue("removeandreturn",null);
                            if (item == null)
                            {
                                continue;
                            }
                        ...
                        else
                        {
                            ...
                            FiverFile file=new FiverFile(currentFile.file,offset,processedBytes);
                            files.add(file);
