                ...
                while (!allFileTransfersCompleted) {
                    ...
                    while (!filesTransferred.isEmpty()) {
                        ...
                        for (int i = 0; i < filesTransferred.size(); i++) {
                            if (InCache(filesTransferred.get(i)))
                                continue;
                            evicted = filesTransferred.get(i);
                            evicted_length = filesTransferredSizes.get(i);
                            break;
                        }
                        if (evicted.equals("nothing") && !allFileTransfersCompleted)
                            continue;
                        else if (evicted.equals("nothing") && allFileTransfersCompleted) {
                            evicted = filesTransferred.get(0);
                        }
                        ...
                        update(evicted, "remove", "filesTransferred", evicted_length);
                        md.reset();
